...
ContainerUI 是一款原生 macOS 桌面应用，为 **[`container`](https://github.com/apple/container)** CLI 运行时提供图形化管理界面——这是 Apple 平台的容器引擎。你可以把它理解为 Apple 容器生态的 "Docker Desktop"：通过简洁的 SwiftUI 界面管理容器、镜像和系统资源。

> **为什么需要 `container`？** ContainerUI 是一个 GUI 前端，它**本身不运行容器**。所有容器操作——创建、启动、停止、构建镜像——都通过调用安装在 `/usr/local/bin/container` 的 [`apple/container`](https://github.com/apple/container) CLI 来完成。没有它，ContainerUI 无法工作。
...
- **生命周期控制** — 通过行内按钮、滑动操作或右键菜单执行启动、停止、强制终止、删除
- **CLI 终端** — 在运行中的容器内执行命令，内置终端界面
...
- **拉取** — 从远程仓库按引用拉取镜像，列表中实时显示进度，失败可重试
- **删除** — 滑动即可删除未使用的镜像

### 镜像构建
- 通过原生文件选择器选择构建上下文目录
- 可指定 Dockerfile 路径（`-f` 参数）
- 为构建结果设置标签
- 实时查看构建日志
...
| **Swift** | 6.0 |
| **Xcode** | 16.0+（推荐） |
| **[`container`](https://github.com/apple/container)** | 安装至 `/usr/local/bin/container` |

> **注意**：`container` CLI 是**必需的依赖**。ContainerUI 通过子进程方式调用它来完成所有操作。若在 `/usr/local/bin/container` 找不到该可执行文件，应用将显示错误提示，所有功能均不可用。
...
```bash
# 参照 https://github.com/apple/container 的说明进行安装
# 安装后确认可执行文件路径
which container
```
...
```
ContainerUI/
├── Package.swift                    # SPM 清单（macOS 26+, Swift 6.0）
├── Sources/
│   └── ContainerUI/
│       ├── App.swift                # @main 入口
...
|------|------|
| **仪表盘** | 系统状态、磁盘使用、版本信息 |
| **容器** | 列出、创建、管理、exec 进入容器 |
| **镜像** | 列出、拉取、删除镜像 |
| **构建** | 从目录或 Dockerfile 构建镜像 |
